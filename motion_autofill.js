(function () {
  ...
  root.innerHTML = [
    ...
    '<p class="ra-note">通常モーション1種と差分モーション最大2種を生成します。Battle Character・スキル・Buster・Arts・Quickは通常/差分とも未入力行を出力しません。EX・宝具は「追加」で必要数だけ増やせます。モーション欄とHit欄は「1行＝1工程」で対応し、Hit欄の数字には出力時に自動で「Hit」を付けます。</p>',
    '<details open>',
  ...
  function pushBattleAndSkill(lines, data, isDiff) {
    ...
    if (anyBattle) {
      ...
      battleKeys.forEach(function (key, idx) {
        ...
        lines.push('|>|第' + (idx + 1) + '段階|>|LEFT:' + plainCell(data[key], false) + '|');
      });
    }
  ...
  function generate() {
    ...
    save(false);
    setStatus('生成しました。未入力のBattle Character・スキル・Q/A/B行は通常/差分とも省略しています。');
  }
